...
const AdoptionRequestItem = ({
  ...
  const onCickStatus = (id: string) => {
    let options: IModalOptions[] = [
      {text: 'Ver Mais Informações', type: 'info'},
    ];

    if (item.status === 'requested') {
      options.push({text: 'Aceitar', type: 'accepted'});
      options.push({text: 'Recusar', type: 'refused'});
    }

    if (item.status === 'accepted') {
      options = [
        {text: 'Entrar em contato', type: 'info'},
        {text: 'Concluir Adoção', type: 'adopted'},
        {text: 'Recusar', type: 'refused'},
      ];
    }

    if (item.status === 'refused') {
      Toast.show('Está solicitação já foi recusada!', Toast.SHORT);
      return null;
    }
